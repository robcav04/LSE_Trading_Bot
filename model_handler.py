...
class ModelHandler:
    ...
    def _build_feature_matrix(self, df_with_indicators: pd.DataFrame, live_open_price: float) -> Optional[np.ndarray]:
        ...
        # 2. CRITICAL FIX: Drop any rows with NaN values that were generated by the
        #    indicator calculations or the shift operation. This mirrors the training script.
        df_cleaned = df_shifted.dropna()

        # 3. Select the final window of data needed for the model from the cleaned data.
        df_window = df_cleaned.tail(self.window)
